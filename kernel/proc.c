...
struct spinlock wait_lock; //added to proc.h as extern
  ...
  for(;;){
    ...
    for(p = proc; p < &proc[NPROC]; p++) {
      ...
      if(p->state == RUNNABLE) {
        ...
        p->state = RUNNING;
        if (p->childnum > 0) {
          p->trapframe->a0 = p->childnum;   // Child returns its own number (1..n)
          p->childnum = 0;                  // Reset so this happens only once
        }
